    ...
    impl TestNode {
        ...
        fn run(self, send_barrier: Arc<Barrier>, drop_barrier: Arc<Barrier>) -> JoinHandle<()> {
            thread!("run!", move || {
                ...
                for _i in 0..(NUM_SERVICES - 1) {
// println!("vault {} waiting for peer {}", self.our_index, _i);
                    let their_id = match unwrap_result!(self.event_rx.recv()) {
                        Event::NewPeer(Ok(()), their_id) => their_id,
                        m => {
                            println!("Expected NewPeer message. Got message {:?}", m);
                            rand::random()
                        }
                    };
                    match their_ids.insert(their_id, 0u32) {
                        Some(_) => println!("Received two NewPeer events for same peer!"),
                        None => (),
                    };
                }
